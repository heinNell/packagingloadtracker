-- Packaging Load Tracker Database Schema
-- PostgreSQL
...
(
    ...
    -- Links to their primary farm/depot
    ...
    updated_at TIMESTAMP
        ...
        -- Add foreign key for user assigned site
    ...
    updated_at TIMESTAMP
            ...
            -- Product types (for tracking what's being transported)
            ...
            (
                ...
                -- e.g., "Citrus", "Stone Fruit", "Vegetables"
                is_active BOOLEAN DEFAULT true,
                created_at TIMESTAMP
                WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
    ...
    updated_at TIMESTAMP
                            ...
                            -- Channels (Retail, Vendor, VanSales, Direct, Municipal, etc.)
    ...
    -- Status tracking
    status VARCHAR
                                (50) NOT NULL DEFAULT 'scheduled' CHECK
                                (status IN
                                (
        'scheduled', 'loading', 'departed', 'in_transit', 
        'arrived_depot', 'unloading', 'completed', 'cancelled'
    )),
    on_time_status VARCHAR
                                (50) CHECK
                                (on_time_status IN
                                ('on_time', 'delayed', 'early')),
    
    -- Depot timing (for multi-stop routes)
    arrived_depot_time TIMESTAMP
                                WITH TIME ZONE,
    departed_depot_time TIMESTAMP
                                WITH TIME ZONE,
    ...
    updated_by UUID REFERENCES users
                                (id),
    confirmed_dispatch_by UUID REFERENCES users
                                (id),
    confirmed_dispatch_at TIMESTAMP
                                WITH TIME ZONE,
    confirmed_receipt_by UUID REFERENCES users
                                (id),
    ...
    updated_at TIMESTAMP
                                ...
                                -- Load packaging items (what packaging is on each load)
                                CREATE TABLE load_packaging
                                (
                                    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    ...
    UNIQUE
                                    (load_id, packaging_type_id, product_type_id, product_variety_id, product_grade_id)
);
                                    ...
                                    CREATE TABLE site_packaging_inventory
                                    (
                                        id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
                                        ...
                                        WITH TIME ZONE,
    last_counted_by UUID REFERENCES users
                                        (id),
    updated_at TIMESTAMP
                                        WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    
    UNIQUE
                                        (site_id, packaging_type_id)
);

                                        -- Packaging movement history (detailed log)
        ...
        'repair', 'purchase', 'disposal', 'transfer'
                                            ...
                                            -- Site packaging thresholds (for alerts)
                                            CREATE TABLE site_packaging_thresholds
                                            (
                                                id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
                                                site_id UUID NOT NULL REFERENCES sites(id),
                                                packaging_type_id UUID NOT NULL REFERENCES packaging_types(id),
    ...
    created_at TIMESTAMP
                                                    ...
                                                    CREATE INDEX idx_loads_dispatch_date ON loads(dispatch_date);
                                                    CREATE INDEX idx_loads_status ON loads(status);
                                                    CREATE INDEX idx_loads_origin ON loads(origin_site_id);
                                                    CREATE INDEX idx_loads_destination ON loads(destination_site_id);
                                                    CREATE INDEX idx_loads_load_number ON loads(load_number);
                                                    CREATE INDEX idx_load_packaging_load ON load_packaging(load_id);
                                                    CREATE INDEX idx_packaging_movements_site ON packaging_movements(site_id);
                                                    CREATE INDEX idx_packaging_movements_date ON packaging_movements(recorded_at);
                                                    CREATE INDEX idx_site_inventory ON site_packaging_inventory(site_id, packaging_type_id);
                                                    ...
                                                    CREATE VIEW v_packaging_in_transit
                                                    AS
                                                        SELECT
                                                    ...
                                                    CREATE VIEW v_site_packaging_summary
                                                    AS
                                                        SELECT
                                                            s.id as site_id,
                                                            s.code as site_code,
...
CROSS JOIN packaging_types pt
                                                    ...
                                                    CREATE VIEW v_load_discrepancies
                                                    AS
                                                        SELECT
                                                            l.id as load_id,
                                                            l.load_number,
                                                    ...
                                                    CREATE VIEW v_overdue_packaging
                                                    AS
                                                        SELECT
                                                            l.id as load_id,
                                                            l.load_number,
